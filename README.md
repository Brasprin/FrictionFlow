...
**Working:** behavioral logging, phase classification, time-in-phase analytics, distraction episodes with resumption times (H1 measure), baseline/intervention conditions, distraction prompt, break mode (tracking suspended during breaks), session interruption/resume, session analytics, real word counts via the Docs API, **AI recovery summaries (Gemini, pre-generated at distraction onset; placeholder content shown when no API key is configured or generation fails)**.
